  ...
  it('should error out on invalid languages inputs', () => {
    assert(
      property(
        ...
        data => {
          expect(
            ruleSchema.validate({ languages: data }).error.details[0].message,
          ).toMatch(
  ...
  it('should error out on invalid languages[language] inputs', () => {
    assert(
      property(
        ...
        data => {
          expect(
            ruleSchema.validate({ languages: { en: data } }).error.details[0]
              .message,
          ).toMatch(
  ...
  it('should error out on invalid languages[language].path inputs', () => {
    assert(
      property(
        ...
        data => {
          expect(
            ruleSchema.validate({ languages: { en: { path: data } } }).error
              .details[0].message,
          ).toMatch(
    ...
    assert(
      property(
        ...
        data => {
          expect(
            ruleSchema.validate({
              languages: { en: { path: 'val', slug: data } },
  ...
  it('should error out on invalid skip inputs', () => {
    assert(
      property(
        ...
        data => {
          expect(
            ruleSchema.validate({ skip: data }).error.details[0].message,
          ).toMatch(/"skip" must be a boolean/i)
        },
      ),
    )
  })
